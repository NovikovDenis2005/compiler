...
public final class AstTransformer {
    ...
    private ConstEnv env = new ConstEnv();
    ...
    private FunctionDecl txFunctionDecl(FunctionDecl f) {
        // Внутри функции нельзя подставлять константы из внешних скоупов: функцию
        // могут вызвать позже, когда внешняя переменная уже изменилась (или она сама
        // её меняет). Поэтому на тело беру отдельное пустое окружение, а внешнее
        // возвращаю обратно после.
        ConstEnv saved = env;
        env = new ConstEnv();
